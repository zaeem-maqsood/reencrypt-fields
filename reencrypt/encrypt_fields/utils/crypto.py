...
class FernetBytes:
    ...
    def __init__(self, key=None, signer=None):
        ...
        self._encryption_key = force_bytes(key or settings.SECRET_KEY[:16])
        self._signer = signer
    ...
    def decrypt(self, data, ttl=None):
        ...
        ciphertext = data[16:]
        keys = [
            force_bytes(settings.SECRET_KEY[:16]),
        ]
        if settings.ROTATED_KEY:
            keys.append(force_bytes(settings.ROTATED_KEY[:16]))
        valid = False
        for key in keys:
            decryptor = Cipher(algorithms.AES(key), modes.CBC(iv), self._backend).decryptor()
