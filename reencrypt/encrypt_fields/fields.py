...
from .core.signing import SignatureExpired
from .utils.crypto import FernetBytes
...
class EncryptedMixin:
    ...
    def _load(self, value):
        try:
            if isinstance(self.base_class(), JSONField):
                return json.loads(pickle.loads(self._fernet.decrypt(value, self.ttl)))
            else:
