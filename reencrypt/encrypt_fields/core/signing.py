...
class FernetSigner(Signer):
    ...
    def __init__(self, key=None):
        ...
        self.keys = [force_bytes(settings.SECRET_KEY)]
        if settings.ROTATED_KEY:
            self.keys.append(force_bytes(settings.ROTATED_KEY))

    def signatures(self, value):
        hs = []
        for key in self.keys:
            h = HMAC(key, self.digest, backend=settings.CRYPTOGRAPHY_BACKEND)
    ...
    def unsign(self, signed_value, ttl=None):
        ...
        valid = False
        for h in self.signatures(signed_value[:-d_size]):
            try:
                h.verify(sig)
                valid = True
